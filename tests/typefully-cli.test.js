...
test('flag parsing: --social-set-id without value errors cleanly', async () => {
	...
	try {
		const result = await runCli(['drafts:list', '--social-set-id'], {
			cwd: sandbox.cwd,
			env: {
				HOME: sandbox.home,
				TYPEFULLY_API_BASE: baseUrl,
...
test('draft target safety: drafts:get single arg with default social set requires --use-default', async () => {
	...
	await fs.writeFile(
		...
		JSON.stringify({ defaultSocialSetId: '111' }, null, 2),
	);

	try {
		const result = await runCli(['drafts:get', 'd99'], {
			cwd: sandbox.cwd,
			env: {
				HOME: sandbox.home,
				TYPEFULLY_API_BASE: baseUrl,
...
test('drafts:get with --use-default uses configured default social set', async () => {
	...
	try {
		const result = await runCli(['drafts:get', 'd99', '--use-default'], {
			cwd: sandbox.cwd,
			env: {
				HOME: sandbox.home,
				TYPEFULLY_API_BASE: baseUrl,
...
test('conflicts: positional social_set_id conflicts with --social-set-id', async () => {
	...
	try {
		const result = await runCli(['drafts:list', '111', '--social-set-id', '222'], {
			cwd: sandbox.cwd,
			env: {
				HOME: sandbox.home,
				TYPEFULLY_API_BASE: baseUrl,
...
test('config:set-default accepts --social-set-id and writes local config', async () => {
	...
	try {
		...
		const cfg = JSON.parse(
			await fs.readFile(path.join(sandbox.cwd, '.typefully', 'config.json'), 'utf8'),
		);
...
test('drafts:create chooses first connected platform when --platform omitted', async () => {
	...
	try {
		const result = await runCli(['drafts:create', '55', '--text', 'Hello'], {
			cwd: sandbox.cwd,
			env: {
				HOME: sandbox.home,
				TYPEFULLY_API_BASE: baseUrl,
...
test('drafts:create --all targets all connected platforms', async () => {
	...
	try {
		const result = await runCli(['drafts:create', '55', '--all', '--text', 'Hello'], {
			cwd: sandbox.cwd,
			env: {
				HOME: sandbox.home,
				TYPEFULLY_API_BASE: baseUrl,
...
test('drafts:schedule sends publish_at payload', async () => {
	...
	try {
		const result = await runCli(['drafts:schedule', '9', 'd1', '--time', 'next-free-slot'], {
			cwd: sandbox.cwd,
			env: {
				HOME: sandbox.home,
				TYPEFULLY_API_BASE: baseUrl,
...
test('tags:list + tags:create hit expected endpoints', async () => {
	...
	try {
		...
		assert.deepEqual(parseJsonOrNull(listRes.stdout), {
			results: [{ id: 't1', name: 'owner/me' }],
		});
...
test('media:upload uses presigned URL and does not set Content-Type on PUT', async () => {
	...
	try {
		const result = await runCli(['media:upload', '9', mediaFilePath, '--no-wait'], {
			cwd: sandbox.cwd,
			env: {
				HOME: sandbox.home,
				TYPEFULLY_API_BASE: baseUrl,
...
test('social-sets:get can use default social set from local config', async () => {
	...
	await fs.writeFile(
		path.join(cfgDir, 'config.json'),
		JSON.stringify({ defaultSocialSetId: '222' }, null, 2),
	);
...
test('drafts:publish sends publish_at=now', async () => {
	...
	try {
		const result = await runCli(['drafts:publish', '9', 'd1'], {
			cwd: sandbox.cwd,
			env: { HOME: sandbox.home, TYPEFULLY_API_BASE: baseUrl, TYPEFULLY_API_KEY: apiKey },
		});
		assert.equal(result.code, 0);
...
test('drafts:delete uses DELETE and returns success payload', async () => {
	...
	try {
		const result = await runCli(['drafts:delete', '9', 'd1'], {
			cwd: sandbox.cwd,
			env: { HOME: sandbox.home, TYPEFULLY_API_BASE: baseUrl, TYPEFULLY_API_KEY: apiKey },
		});
		assert.equal(result.code, 0);
...
test('media:status hits /media/<id> with default social set when omitted', async () => {
	...
	await fs.writeFile(
		path.join(cfgDir, 'config.json'),
		JSON.stringify({ defaultSocialSetId: '9' }, null, 2),
	);
	...
	try {
		const result = await runCli(['media:status', 'm1'], {
			cwd: sandbox.cwd,
			env: { HOME: sandbox.home, TYPEFULLY_API_BASE: baseUrl, TYPEFULLY_API_KEY: apiKey },
		});
		assert.equal(result.code, 0);
...
test('drafts:update with no update fields errors (message mentions --tags)', async () => {
	...
	try {
		const result = await runCli(['drafts:update', '9', 'd1'], {
			cwd: sandbox.cwd,
			env: { HOME: sandbox.home, TYPEFULLY_API_BASE: baseUrl, TYPEFULLY_API_KEY: 'typ_test_key' },
		});
		assert.equal(result.code, 1);
		assert.deepEqual(parseJsonOrNull(result.stdout), {
			error:
				'At least one of --text, --file, --title, --schedule, --share, --notes, or --tags is required',
		});
		assert.equal(server.requests.length, 0);
	} finally {
		await server.close();
...
test('drafts:update can clear tags with --tags "" (sends empty array)', async () => {
	...
	try {
		const result = await runCli(['drafts:update', '9', 'd1', '--tags', ''], {
			cwd: sandbox.cwd,
			env: { HOME: sandbox.home, TYPEFULLY_API_BASE: baseUrl, TYPEFULLY_API_KEY: apiKey },
		});
		assert.equal(result.code, 0);
...
test('drafts:update replaces posts and attaches media only to first post', async () => {
	...
	server.expect('PATCH', '/v2/social-sets/9/drafts/d1', {
		assert: (req) => {
			authAssertFactory(apiKey)(req);
			const posts = req.bodyJson.platforms.x.posts;
			assert.deepEqual(posts, [{ text: 'First', media_ids: ['m1'] }, { text: 'Second' }]);
		},
		json: { id: 'd1', ok: true },
	});
...
test('drafts:create reads from file via -f', async () => {
	...
	try {
		const result = await runCli(['drafts:create', '9', '--platform', 'x', '-f', filePath], {
			cwd: sandbox.cwd,
			env: { HOME: sandbox.home, TYPEFULLY_API_BASE: baseUrl, TYPEFULLY_API_KEY: apiKey },
		});
		assert.equal(result.code, 0);
...
test('drafts:create errors when neither --text nor --file are provided', async () => {
	const sandbox = await makeSandbox();
	try {
		const result = await runCli(['drafts:create', '9', '--platform', 'x'], {
			cwd: sandbox.cwd,
...
test('drafts:update supports -a shorthand for --append', async () => {
	...
	try {
		const result = await runCli(['drafts:update', '9', 'd1', '-a', '--text', 'New'], {
			cwd: sandbox.cwd,
			env: { HOME: sandbox.home, TYPEFULLY_API_BASE: baseUrl, TYPEFULLY_API_KEY: apiKey },
		});
		assert.equal(result.code, 0);
...
test('drafts:schedule/delete/publish work with --use-default and configured default social set', async () => {
	...
	try {
		const sched = await runCli(
			['drafts:schedule', 'd1', '--time', 'next-free-slot', '--use-default'],
			{
				cwd: sandbox.cwd,
				env: { HOME: sandbox.home, TYPEFULLY_API_BASE: baseUrl, TYPEFULLY_API_KEY: apiKey },
			},
...
test('media:upload polling path works (uses --timeout and fast poll interval override)', async () => {
	...
	try {
		const result = await runCli(['media:upload', '9', mediaFilePath, '--timeout', '1'], {
			cwd: sandbox.cwd,
			env: {
				HOME: sandbox.home,
				TYPEFULLY_API_BASE: baseUrl,
...
test('setup supports --scope as alias for --location (non-interactive)', async () => {
	...
	try {
		...
		assert.equal(cfg.apiKey, 'typ_setup_key');
		assert.ok(
			(await fs.readFile(path.join(sandbox.cwd, '.gitignore'), 'utf8')).includes('.typefully/'),
		);
	} finally {
		await sandbox.cleanup();
	}
...
test('drafts:create reads from file via --file (long form)', async () => {
	...
	try {
		const result = await runCli(['drafts:create', '9', '--platform', 'x', '--file', filePath], {
			cwd: sandbox.cwd,
			env: { HOME: sandbox.home, TYPEFULLY_API_BASE: baseUrl, TYPEFULLY_API_KEY: apiKey },
		});
		assert.equal(result.code, 0);
...
test('media:upload uses configured default social set when only file arg is provided', async () => {
	...
	try {
		const result = await runCli(['media:upload', mediaFilePath, '--no-wait'], {
			cwd: sandbox.cwd,
			env: { HOME: sandbox.home, TYPEFULLY_API_BASE: baseUrl, TYPEFULLY_API_KEY: apiKey },
		});
		assert.equal(result.code, 0);
...
test('drafts:schedule errors when --time is missing', async () => {
	...
	try {
		const result = await runCli(['drafts:schedule', '9', 'd1'], {
			cwd: sandbox.cwd,
			env: { HOME: sandbox.home, TYPEFULLY_API_BASE: baseUrl, TYPEFULLY_API_KEY: 'typ_test_key' },
		});
		assert.equal(result.code, 1);
...
test('global flag: drafts:list accepts --social-set-id (kebab-case)', async () => {
	...
	try {
		const result = await runCli(['drafts:list', '--social-set-id', '9'], {
			cwd: sandbox.cwd,
			env: { HOME: sandbox.home, TYPEFULLY_API_BASE: baseUrl, TYPEFULLY_API_KEY: apiKey },
		});
		assert.equal(result.code, 0);
...
test('global flag: tags:list accepts --social-set-id (kebab-case) with no positional social set', async () => {
	...
	try {
		const result = await runCli(['tags:list', '--social-set-id', '9'], {
			cwd: sandbox.cwd,
			env: { HOME: sandbox.home, TYPEFULLY_API_BASE: baseUrl, TYPEFULLY_API_KEY: apiKey },
		});
		assert.equal(result.code, 0);
